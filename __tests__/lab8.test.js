describe('Basic user flow for Website', () => {
  ...
  it('Clicking the "Add to Cart" button should change button text', async () => {
    ...
    const root = await productItem.getProperty('shadowRoot');
    const button = await root.$('button');
    await button.click();
    let text = await button.getProperty('innerText');
    text = await text.jsonValue();
    const textValue = text === "Remove from Cart" ? true : false;
    expect(textValue).toBe(true);
    await button.click();
  }, 2500);
  ...
  it('Checking number of items in cart on screen', async () => {
    ...
    let countValue = await count.getProperty('innerText');
    countValue = await countValue.jsonValue();
  ...
  it('Checking number of items in cart on screen after reload', async () => {
    ...
    for (let i = 0; i < prodItems.length; i++) {
      ...
      let text = await button.getProperty('innerText');
      text = await text.jsonValue();
      const textValue = text === "Add to Cart" ? true : false;
      expect(textValue).toBe(true);
    }
